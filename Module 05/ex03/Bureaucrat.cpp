...
/*   Updated: 2023/12/08 11:44:51 by afatir           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::signForm(std::string name, bool sign) const
{
	if (sign)
		std::cout << this->_name << " signed " << name << std::endl;
	else
		std::cout << this->_name << " couldn't sign " << name << " because " << this->_name << " grade is too low." << std::endl;
}

void	Bureaucrat::executeForm(std::string name, bool sign) const
{
	if (sign)
		std::cout << this->_name << " executes " << name << std::endl;
	else
		std::cout << this->_name << " couldn't execute " << name << " because " << this->_name << " grade is too low." << std::endl;
}
